...
export class RightsPage {
    ...
    async openTab() {
        await this.page.getByRole('button', { name: 'Rights, licencing and re-use' }).click();
    }

    async setRights(){
        const rightsHolderField = this.page.getByTestId(RegExp('rightsholder')).nth(1)
        rightsHolderField.click()
        rightsHolderField.fill('DANS Org')
        await loadMockDataDropdown(this.page, 'rights.json', '**/expanded-search/?q=*')
