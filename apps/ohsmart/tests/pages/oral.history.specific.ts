...
export class OralHistorySpecificPage {
  ...
  async openTab() {
    await this.page
      .getByRole("button", { name: "Oral-history specific" })
      .click();
  }
  ...
  async setDateAndTime(numberOfDates: number) {
    ...
    await createMany(
      this.page,
      "add-button-interview_date_time",
      numberOfDates,
    );
  ...
  async setTranscribers(numberOfTranscribers: number) {
    await createMany(
      this.page,
      "add-button-transcript_human",
      numberOfTranscribers,
    );
    const groupFields = generatePersonGroupFields("transcript_human", [
      "function",
    ]);
  ...
  async setSectionLessElements() {
    await clickAndFillApiDropdown(
      this.page,
      "interview_location",
      "The Netherlands (independent political entity) The Netherlands",
      "country.json",
      "**/searchJSON?q=*&username=*",
    );
    ...
    await this.page
      .getByTestId(RegExp("recording_format"))
      .first()
      .pressSequentially("mp3");
 ...
 * @returns GroupField
 */
function generatePersonGroupFields(
  role: string,
  ...
  const additionalFieldNames = [
    ...new Set([...defaultFieldNames, ...additionalFields]),
  ];
