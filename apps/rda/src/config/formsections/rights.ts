import type { InitialSectionType } from "@dans-framework/deposit";

const section: InitialSectionType = {
  id: "rights",
  title: {
    en: "Rights, licencing and re-use",
    nl: "Rechten, licenties en hergebruik",
  },
  fields: [
    {
      type: "group",
      name: "rightsholders",
      label: {
        en: "Rights holders",
        nl: "Rechthebbenden",
      },
      description: {
        en: "The organisation or individual(s) owning the work",
        nl: "De organisatie of personen die eigenaar zijn van het werk",
      },
      repeatable: true,
      fields: [
        {
          type: "autocomplete",
          name: "rightsholder",
          label: {
            en: "Rights holder",
            nl: "Rechthebbende",
          },
          required: true,
          description: {
            en: "Name of the organisation owning the work. RDA is the default rights holder for RDA outputs.",
            nl: "Naam van de organisatie die eigenaar is van het werk. RDA is de standaard rechthebbende voor RDA outputs.",
          },
          multiApiValue: "ror",
          options: ["ror", "orcid"],
          value: {
            label: "Research Data Alliance",
            value: "https://ror.org/05dc6w374",
            extraLabel: "country",
            extraContent: "United Kingdom",
          },
        },
      ],
    },
    {
      type: "autocomplete",
      name: "accessTypes",
      label: {
        en: "Access types",
        nl: "Toegangstypen",
      },
      required: true,
      description: {
        en: "The access types that apply to this deposit",
        nl: "De toegangstypen die van toepassing zijn op dit deposit",
      },
      options: [
        {
          label: {
            en: "Open",
            nl: "Open",
          },
          value: "open",
        },
        {
          label: {
            en: "Embargoed",
            nl: "Embargoed",
          },
          value: "embargoed",
        },
        {
          label: {
            en: "Restricted",
            nl: "Beperkt",
          },
          value: "restricted",
        },
        {
          label: {
            en: "Closed",
            nl: "Gesloten",
          },
          value: "closed",
        },
      ],
      value: { label: "Open", value: "open" },
    },
    {
      type: "autocomplete",
      name: "licence",
      label: {
        en: "Licence",
        nl: "Licentie",
      },
      required: true,
      description: {
        en: "One of a number of specific licences",
        nl: "Eén of meerdere specifieke licenties",
      },
      options: "licenses",
      value: {
        label: "CC-BY-4.0 Creative Commons Attribution 4.0",
        value: "https://creativecommons.org/licenses/by/4.0/",
        id: "CC-BY-4.0",
      },
    },
  ],
};

export default section;
