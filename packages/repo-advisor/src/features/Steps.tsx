        ...
        {depositType === "dataset" && (
          ...
          >
            <SelectField
              ...
              options={[
                {
                  label: t("audiovisual_materials"),
                  value: "audiovisual_materials",
                },
                { label: t("statistical_data"), value: "statistical_data" },
                {
                  label: t("geospatial_data_files"),
                  value: "geospatial_data_files",
                },
                {
                  label: t("netcdf_and_hdf_files"),
                  value: "netcdf_and_hdf_files",
                },
                {
                  label: t("darwin_core_and_ecological_markup_language_files"),
                  value: "darwin_core_and_ecological_markup_language_files",
                },
...
export const Step2 = () => {
  ...
  return (
    <StepWrap
      title={t("selectDataset")}
      subtitle={t("selectDatasetDescription")}
    >
      {data && data.length > 0 ?
        <List sx={{ mb: 2 }}>
          {data.map((rec, i) => (
              ...
              >
                <ListItemIcon>
                  <Checkbox
                    edge="start"
                    checked={
                      (repo?.displayName as LanguageStrings)?.en ===
                      (rec?.displayName as LanguageStrings)?.en
                    }
                    tabIndex={-1}
                  />
                </ListItemIcon>
