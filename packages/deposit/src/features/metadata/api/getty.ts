...
export const gettyApi = createApi({
  ...
  endpoints: (build) => ({
    fetchGettyAATTerms: build.query({
      query: (content) => {
        const search =
          content.replace(/ +(?= |$)/g, "").replace(/ +/g, " AND ") + "*";
        return {
          url: `proxy?url=${encodeURIComponent(
            `http://vocabsservices.getty.edu/AATService.asmx/AATGetTermMatch?term=${search}&logop=and&notes=`,
          )}`,
      ...
      transformErrorResponse: () => ({
        error: i18n.t("metadata:apiFetchError", { api: "Getty AAT" }),
      }),
    }),
  }),
});

export const { useFetchGettyAATTermsQuery } = gettyApi;
